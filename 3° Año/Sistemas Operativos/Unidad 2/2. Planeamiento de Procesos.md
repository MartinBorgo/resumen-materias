...
- **Round Robin**: Consiste en conceder a cada proceso en ejecución un determinado período de tiempo ***q(quantum)***, luego del cual si el proceso no terminó se lo devuelve al final de la cola de procesos listos.
...
- **Planificación por Prioridades**: Se asigna el procesador al proceso con mayor prioridad. El principal problema de esta política es la postergación indefinida de los procesos con baja prioridad, para solucionar esto existe el ***envejecimiento de las prioridades*** que aumenta gradualmente las prioridades de los procesos listos y ante iguales prioridades atiende al proceso más antiguo. Este método se puede emplear de dos formas:
	- Sin desalojo.
	- Con desalojo
