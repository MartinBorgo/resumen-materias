...
Si el error ocurre durante la fase de preparación, sucede la siguiente secuencia de eventos:
...
>
>- ***OK*** si el nodo tiene un $r-token$ o ningún token, o si tiene un $w-token$ pero no necesita. En tal caso destruye el token de escritura para luego responder a la petición de afirmativamente.
...
Los algoritmos de prevención y detección de interbloqueos pueden adaptarse a sistemas distribuidos, aunque requieren modificaciones. Pero dado que la prevención de interbloqueos introduce esperas y retrocesos innecesarios, además de involucrar más sitios de los necesarios en la ejecución de una transacción se suele optar por otros enfoques.
