...
- Testear relaciones corroborando si estas son legales para un determinado conjunto de dependencias funcionales dadas. Si una relación $r$ es legal para un conjunto $F$ de dependencias funcionales se dice que $r$ satisface a $F$.
...
Dos conjuntos de dependencias son equivalentes si $F_1^+ = F_2^+ \Rightarrow F_1 \equiv F_2$. Pero como no es conveniente calcular las clausuras para cada $F_i$, se puede chequear las equivalencias de la siguiente manera $F_1 \vdash F_2 \land F_2 \vdash F_1 \Rightarrow F_1 \equiv F_2$, es decir, todas las dependencias funcionales de $F_2$ pueden ser inferidas a partir de $F_1$ y viceversa, si esto es posible para ambos conjuntos entonces $F_1$ es una cobertura canónica de $F_2$.
#### Atributos Extraños
...
1. Para testear si el atributo $C \in X$ es extraño en $X$, primero se calcula $(X - C)^+$ usando las dependencias del conjunto $F$, luego se chequea si esta clausura contiene a $C$, si es así $C$ es extraño en $X$.
2. Para testear si el atributo $C \in Y$ es extraño en $Y$, primero se calcula $X^+$ usando el siguiente conjunto $F' = F - \{X \rightarrow Y\} \cup \{X \rightarrow (Y - C)\}$, luego se chequea si $X^+$ en $F'$ contiene a $C$, si es así, $C$ es extraño.
