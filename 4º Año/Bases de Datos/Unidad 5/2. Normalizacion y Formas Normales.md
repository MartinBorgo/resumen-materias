Las metas de la normalización es decidir si un esquema de relación $R$ está en buena forma. En caso de que esto no sea así se divide el esquema en subesquemas de la forma $\{R_1, R_2, ... , R_n\}$ tal que, cada subesquema de la relación $R$ este en buena forma y la descomposición sea de join sin perdidas. Cabe aclarar que para que la descomposición sea considerada válida se debe verificar que $R_1 \cup R_2 \cup R_n = R$.
...
- Cada atributo $A \in Y - X$ está contenido en alguna clave candidata de $R$. Cada atributo puede estar en diferentes claves primarias.
...
3. Para cada par de dependencias funcionales de la forma $X \rightarrow Y$ se creará una nueva relación $R_i(X, Y)$, es decir, se creará una nueva relación que contenga todos los atributos tanto del lado derecho como del lado izquierdo. En caso de que $XY$ ya formen parte de un subesquema entonces no se deberá crear una nueva relación.
4. Si alguna de las $R_i$ relaciones contiene al menos a una clave candidata el proceso se detiene y $R$ está ya en tercera forma normal. En caso contrario se deberá crear una nueva relación $R_i$ que contenga a cualquiera de las claves candidatas.
...
Generalmente, el proceso de diseño de una base de datos comienza por el modelo conceptual de este mismo a través de un diagrama de Entidad-Relación, para luego convertir este mismo en tablas, obteniendo así nuestro esquema $R$. Este esquema $R$ puede haber sido una relación simple conteniendo todos los atributos de interés y luego se le aplica normalización descomponiendo $R$ en subesquemas.