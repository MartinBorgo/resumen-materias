...
Veamos ahora la siguiente fórmula $(\exists x)(\forall y)(\exists z)(A(x) \wedge (B(y) \vee C(z)))$ en este caso la fórmula skolemizada nos quedaría $(\forall y)(A(a) \wedge (B(y) \vee C(sk_3(y)))$ en este caso $a$ es una constante arbitraria que no aparecía en la matriz de $A$ y  $sk3$ es una función de Skolem.
