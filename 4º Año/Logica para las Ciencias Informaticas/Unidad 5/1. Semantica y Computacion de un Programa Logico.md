...
Este tipo de cláusulas suelen recibir el nombre de consultas o cláusulas objetivo. Nótese que en este caso también estamos en presencia de una cláusula de Horn.
Estas metas definidas son equivalentes a $¬B_1 \vee ¬B_2 \vee \dots \vee ¬B_n$ y en ellas se asume que todas las variables de las cláusulas están cuantificadas universalmente:
...
Los sistemas que utilizan programación en lógica están basados en la regla de resolución como única regla de inferencia. Esto significa que sí se trata de probar que $\exists(B_1 \wedge B_2 \wedge \dots \wedge B_n)$ es una consecuencia de un programa $P$, se niega esa fórmula obteniendo una meta definida $\leftarrow B_1, B_2, \dots, B_n$, la cual definimos con anterioridad.
...
Del teorema de completitud se deduce que la resolución SLD es un procedimiento completo independientemente de la elección de la regla de computación seleccionada, pero solo indica que existe una refutación.
La elección de la regla de búsqueda determinará en que la refutación se encuentre o no, existen dos métodos de búsqueda:
...
Dado un conjunto de cláusulas $S$, definimos el ***Universo o Dominio de Herbrand*** de $S$ como el conjunto de términos $\mathscr{H}_S$ obtenidos a partir de $S$ de la siguiente manera:
...
>El universo o dominio de Herbrand pueden ser finitos, pero si en $S$ existe al menos una letra funcional, entonces el universo de Herbrand es infinito. En caso de que se necesite ver un ejemplo ir a las filminas de la cátedra.

Dado un conjunto de cláusulas $S$ definimos la ***Base de Herbrand*** de $S$, denotada como $\mathscr{B}_S$, como el conjunto de literales siguiente:
...
>En términos computacionales este teorema sugiere un método para verificar la satisfabilidad de un conjunto de cláusulas. Es posible generar de manera parcial, de forma iterativa aplicando $\Re$, el universo de Herbrand y realizar las correspondientes substituciones para obtener parte de la expansión de Herbrand, si fuera inconsistente entonces $S$ sería insatisfacible.
## Significado en el Cálculo de Predicados
Sea $P$ un programa lógico. El significado de $P$, denotado como $M(P)$, es un conjunto de metas fijas deducibles de $P$. A partir de esta definición se obtiene que si un programa lógico $P$ solo contiene hechos si significado es el mismo $P$.
...
Dado un conjunto de fórmulas cerradas $S$, los elementos lingüísticos involucrados en sus cláusulas, es decir, sus constantes, letras funcionales y predicativas, definen un lenguaje particular que se denomina el lenguaje de $S$.
Sea $S$ un conjunto de fórmulas cerradas. Una interpretación de Herbrand de $S$ es una interpretación de Herbrand del lenguaje de $S$. Un modelo de Herbrand de $S$ es una interpretación de Herbrand de $S$ que es un modelo de $S$. Dado un programa lógico $P$, una interpretación de Herbrand de $P$ es una interpretación de Herbrand del lenguaje de $P$.
...
>Entonces podemos llegar a la conclución de que $M_P = M(P)$.
...
Es posible probar que, para programas definidos, siempre existe una interpretación mínima $\mathscr{I}$ tal que $T_P(\mathscr{I}) = \mathscr{I}$ y que esa interpretación I es también el modelo mínimo de Herbrand $M_P$. Además, $M_P$ es el límite de una secuencia creciente y posiblemente infinita de iteraciones:
$$\varnothing, \  T_P(\varnothing), \ T_P(T_P(\varnothing)), \ T_P(T_P(\varnothing)), \ \dots$$
