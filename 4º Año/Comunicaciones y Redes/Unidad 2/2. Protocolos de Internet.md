...
Es una ampliación del RFC 822 para resolver problemas que acarreaba SMTP. Este protocolo permite la transferencia de archivos ejecutables y binarios, mensajes de cualquier tamaño y otros códigos, además de ASCII. En su especificación se definen nuevos campos en la cabecera del mensaje, formatos de contenido (Texto sin formato, Imágenes, Audio, Video, Aplicación, Multiparte) y esquemas de codificación para interpretar cualquier formato deseado. La codificación de los mensajes en MIME se realiza con Radix-64, donde se codifican los datos para que no sean vulnerables al procesarse por los programas de transporte de correo.
MIME no es un protocolo por si mismo, sino que es una mejora SMTP el cual solo agrega campos extras a los encabezados, lo que le permite enviar una gama más amplia de información. Además de implementar una mayor seguridad utilizando cifrado para sus mensajes.
