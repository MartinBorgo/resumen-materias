...
- Los servidores alojan objetos web direccionables por una URL, por ejemplo, Apache-Microsoft Internet Information Server.
...
Este protocolo tiene una particularidad, ya que utiliza dos conexiones TCP simultáneas para transferir archivos, una se utiliza para realizar el control de los datos (a través del puerto 21), es decir, chequeo y control de errores y la segunda para la transferencia de datos como tal, como esta información de control no se envía por la misma conexión TCP se dice que esta información se envía ***fuera de banda***.
