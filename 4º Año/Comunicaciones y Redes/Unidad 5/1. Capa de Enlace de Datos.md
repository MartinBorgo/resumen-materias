...
Para cumplir con estas metas, la capa de enlace de datos toma los paquetes que obtiene de la capa de red y los encapsula en ***tramas*** para transmitirlos. Cada trama contiene un encabezado, un campo de carga útil y un terminador o cola.
Los distintos protocolos de la capa de enlace define el formato de las tramas intercambiadas entre los nodos situados en los extremos del enlace, así como las acciones que estos llevan a cabo cuando se envían y reciben las tramas.
El servicio básico de cualquier capa de enlace es mover una trama desde un nodo hasta otro adyacente a través de un único enlace de comunicaciones, aunque los detalles del servicio proporcionado pueden variar de un protocolo de la capa de enlace a otro. Muchos de los servicios proporcionados por la capa de enlace presentan notables paralelismos con los servicios de la capa de transporte, no obstante no son lo mismo, ya que están construidos para fines diferentes.
...
2. **Servicio sin conexión con confirmación de recepción**: El emisor envía tramas sin establecer una conexión lógica entre los nodos, pero el receptor confirma de manera individual la recepción de cada trama enviada. De esta manera, el emisor sabe si la trama llegó correctamente. Si no ha llegado en un intervalo especificado, se puede reenviar la trama nuevamente. Este servicio es útil en canales no confiables, como los de los sistemas inalámbricos.
...
El protocolo de ventana deslizante es un mecanismo de control el flujo que permite al emisor enviar múltiples tramas antes de necesitar una confirmación (ACK), aumentando así la eficiencia y la utilización del canal al máximo. Tanto el emisor como el receptor tienen una “ventana” que indica el número de tramas que se pueden transmitir sin confirmar. El emisor envía un número $w$ de tramas, cada una con un número de secuencia. El receptor recibe las tramas y envía ACKs para las tramas correctas. Al recibir un ACK, la ventana del emisor se desplaza, permitiendo enviar nuevas tramas. Tradicionalmente, los datos y tramas de control se enviaban por enlaces físicos separados, lo que era menos eficiente, pero con el uso de conexiones _full-duplex_ o _half-duplex_, las tramas se intercalan en el mismo enlace.
...
>A la hora de enviar ACK al emisor, el receptor no necesita enviar una confirmación para todas las tramas, solo basta con enviar cuál es el número de secuencia de la trama que está esperando (Receive Ready).
...
Una técnica detección de errores utilizada ampliamente en las redes de computadoras de hoy día está basada en los ***códigos de comprobación de redundancia cíclica (CRC)***.
...
Todas las técnicas de corrección de errores agregan cierta redundancia en las tramas enviadas, para que el receptor al detectar un error puede corregir este mismo en ese instante. Entre los códigos utilizados para la corrección de errores se encuentran los siguientes:
...
Como veremos más adelante cada tipo de red tiene su propio protocolo y formato de trama, nosotros nos centraremos en estos momentos en explicar de la forma más resumida posible dos protocolos muy importantes, HDLC que es utilizado en muchos casos como punto de referencia para el desarrollo del resto de protocolos LAN. El segundo de estos protocolos es ARP utilizado principalmente para realizar la correspondencia entre direcciones IP y direcciones MAC.
